...
export function setupAllEventListeners() {
    ...
    getSlotIncreaseBtn().addEventListener('click', () => updateInventorySlotCount(getState().slotCount + 1));
    getSlotDecreaseBtn().addEventListener('click', () => updateInventorySlotCount(getState().slotCount - 1));
    ...
    getInventoryGrid().addEventListener('click', e => {
        ...
        if (getState().inventory[slotId]) {
            ...
            if (firstOwnedItemId) {
                placeItemInSlot(slotId, firstOwnedItemId); // ID만 전달
            }
        }
    });
