...
function renderItems() {
    ...
    db.filter(item => {
        ...
        const count = ownedItems[item.id]?.count || 0; // ★★★ 수정
...
function updateSelectedItems() {
    ...
    // ownedItems (보유 아이템)을 순회
    Object.keys(ownedItems).forEach(itemId => {
        const itemState = ownedItems[itemId]; // { item, count, upgrade, priority }
        ...
        if (!isArtifact) {
            ...
            // 아티팩트 렌더링 (강화/중요도 UI 포함)
            const card = document.createElement('div');
            card.className = 'selected-item-card';
            card.dataset.itemId = dbItem.id; // ★★★ inventoryIndex 대신 itemId 사용 ★★★
            card.innerHTML = `
